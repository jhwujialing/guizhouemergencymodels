...
class ContinuousModel(object):
    ...
    def getNd(self, x, y, z, t):
        ...
        if self.H != 0:
            ...
            f = lambda dt: (int(1)-float(0.007)*(dt**0.5))*a/b*d*c*math.exp(-(x-self.u*(t-dt))*(x-self.u*(t-dt))/(int(2)*Xksxs*Xksxs))
        else:
            #f = lambda dt: int(2)*a/b*d*math.exp(-(x-self.u*(t-dt))*(x-self.u*(t-dt))/(int(2)*Xksxs*Xksxs))
            f = lambda dt: (int(1)-float(0.007)*(dt**0.5))*a/b*d*math.exp(-(x-self.u*(t-dt))*(x-self.u*(t-dt))/(int(2)*Xksxs*Xksxs))
        #print(t, self.tr)
        if t <= self.tr:
